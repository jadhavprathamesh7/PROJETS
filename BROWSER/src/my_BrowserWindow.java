...
/**
 * This package-private class defines one my_browser window for use with 
 ...
 */
class my_BrowserWindow extends Stage {

    private my_WebBrowser my_owner;    // the WebBrowser application
    private WebEngine my_webEngine; // loads and manages pages
    private Menu my_windowMenu;     // holds window-related commands

    /**
     * The constructor sets up the window, and starts loading the
     * initial web page location, if given.  It does not show the window.
     * @param my_browser     the WebBrowser application that is createing this window
     * @param my_initialURL  if non-null, the window will attempt to load this URL
     */
    my_BrowserWindow(my_WebBrowser my_browser, String my_initialURL) {
        ...
        my_webEngine = webview.getEngine();
        ...
        my_webEngine.locationProperty().addListener( (o,oldVal,newVal) -> {
        ...
        my_webEngine.titleProperty().addListener( (o,oldVal,newVal) -> {
            if (newVal == null)
                setTitle("Untitled " + my_owner.my_getNextUntitledCount());
            else
                setTitle(newVal); 
        });
        ...
        my_webEngine.getLoadWorker().stateProperty().addListener( (o,oldVal,newVal) -> {
        ...
        cancelButton.setOnAction( e -> { // Cancel the ongoing page load
            if ( my_webEngine.getLoadWorker().getState() == Worker.State.RUNNING )
                my_webEngine.getLoadWorker().cancel();
        ...
        my_webEngine.setOnAlert( evt -> SimpleDialogs.message(evt.getData(), "Alert from web page") );
        my_webEngine.setPromptHandler( promptData -> 
                    SimpleDialogs.prompt(promptData.getMessage(), 
                                              "Query from web page", promptData.getDefaultValue()));
        my_webEngine.setConfirmHandler( str -> 
                    SimpleDialogs.confirm(str, "Confirmation Needed").equals("yes") );
        
        /* If the my_initialURL is not null, load that page! */
        
        if (my_initialURL != null) {
               // load the initial web page, if any
            doLoad(my_initialURL);
        }
    ...
    private MenuBar makeMenuBar() {
        ...
        newWin.setOnAction( e -> my_owner.my_newBrowserWindow(null) );
        ...
        open.setOnAction( e -> {
            ...
            if (url != null && url.trim().length() > 0)
                my_owner.my_newBrowserWindow(url);
        });
        my_windowMenu = new Menu("Window");
        my_windowMenu.getItems().addAll(newWin,close,open,new SeparatorMenuItem());
        my_windowMenu.setOnShowing( e -> populateWindowMenu() );
        MenuBar menubar = new MenuBar(my_windowMenu);
        return menubar;
    }
    ...
    private void populateWindowMenu() {
        ArrayList<my_BrowserWindow> windows = my_owner.getOpenWindowList();
        while (my_windowMenu.getItems().size() > 4) {
            ...
            my_windowMenu.getItems().remove(my_windowMenu.getItems().size() - 1);
        }
        if (windows.size() > 1) {
            ...
            my_windowMenu.getItems().add(item);
            my_windowMenu.getItems().add( new SeparatorMenuItem() );
        }
        for (my_BrowserWindow window : windows) {
            ...
            final my_BrowserWindow win = window; // (for use in a lambda expression)
